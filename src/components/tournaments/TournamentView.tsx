...
// Temporary stub until TOURNAMENTS is implemented
...
export const TournamentView: React.FC<TournamentViewProps> = ({
  userDogs: _userDogs,
  onEnterTournament: _onEnterTournament,
}) => {
  const [selectedTab, setSelectedTab] = useState<'upcoming' | 'active' | 'completed'>('upcoming');
  const [selectedTournament, setSelectedTournament] = useState<Tournament | null>(null);

  const upcomingTournaments = TOURNAMENTS.filter(t =>
    new Date((t as any).registrationDeadline) > new Date()
  );

  const renderTournamentCard = (tournament: Tournament) => {
    const t = tournament as any;
    const isRegistrationOpen = new Date(t.registrationDeadline) > new Date();
    const hasStarted = new Date(t.startsAt || t.startDate) <= new Date();
      ...
      >
        <div className="flex justify-between items-start mb-4">
          ...
          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
            t.tier === 'championship' ? 'bg-purple-100 text-purple-800' :
            t.tier === 'national' ? 'bg-blue-100 text-blue-800' :
            t.tier === 'regional' ? 'bg-green-100 text-green-800' :
            'bg-gray-100 text-gray-800'
          }`}>
            {(t.tier || 'local').toUpperCase()}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <p className="font-semibold text-green-600">${t.prizePool?.first || 0}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t">
          <div className="text-sm">
            ...
            <p className="text-gray-500">
              {isRegistrationOpen ? (
                <>Registration closes: {new Date(t.registrationDeadline).toLocaleDateString()}</>
              ) : (
                <>Starts: {new Date(t.startsAt || t.startDate).toLocaleDateString()}</>
              )}
            </p>
          </div>
  ...
  const renderTournamentDetails = () => {
    ...
    const st = selectedTournament as any; // Cast to any for incomplete Tournament type
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-xl font-bold text-green-600">${st.prizePool.first}</p>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-4 gap-3">
                <div className="bg-yellow-50 p-3 rounded-lg text-center">
                  ...
                  <p className="font-bold text-yellow-700">${st.prizePool.first}</p>
                </div>
                <div className="bg-gray-50 p-3 rounded-lg text-center">
                  <p className="text-sm text-gray-600">🥈 2nd</p>
                  <p className="font-bold text-gray-700">${st.prizePool.second}</p>
                </div>
                <div className="bg-orange-50 p-3 rounded-lg text-center">
                  <p className="text-sm text-gray-600">🥉 3rd</p>
                  <p className="font-bold text-orange-700">${st.prizePool.third}</p>
                </div>
                {st.prizePool.fourth && (
                  <div className="bg-blue-50 p-3 rounded-lg text-center">
                    <p className="text-sm text-gray-600">4th</p>
                    <p className="font-bold text-blue-700">${st.prizePool.fourth}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Entry Requirements */}
            {st.entryRequirements && (
              <div className="mb-6">
                <h3 className="font-semibold text-gray-800 mb-3">Entry Requirements</h3>
                <ul className="space-y-2">
                  {st.entryRequirements.minLevel && (
                    <li className="flex items-center text-sm">
                      <span className="w-2 h-2 bg-blue-500 rounded-full mr-2"></span>
                      Minimum Level: {st.entryRequirements.minLevel}
                    </li>
                  )}
                  {st.entryRequirements.minCompetitionWins && (
                    <li className="flex items-center text-sm">
                      <span className="w-2 h-2 bg-blue-500 rounded-full mr-2"></span>
                      Minimum Competition Wins: {st.entryRequirements.minCompetitionWins}
                    </li>
                  )}
                  {st.entryRequirements.requiredCertifications && (
                    <li className="flex items-center text-sm">
                      <span className="w-2 h-2 bg-blue-500 rounded-full mr-2"></span>
                      Required Certifications: {st.entryRequirements.requiredCertifications.join(', ')}
                    </li>
                  )}
                </ul>
              </div>
